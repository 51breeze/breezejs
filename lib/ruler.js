...
function getImportClassName(name, scope )
{
     ...
     return {id:'class','type':'('+name+')','classname':name, 'fullclassname':name};
}
...
function importClass( classname, filename , scope )
{
    ...
    var desc = {'type':'('+classname+')','id':'class','fullclassname':full,'classname':classname };
...
syntax['import']=function (event)
{
    ...
    if( this.scope().keyword() !=='package' )
    {
        if( this.strict() )this.error('Unexpected import ', 'syntax');
        if( this.scope().keyword() !=='rootblock' )this.error();
...
syntax['private,protected,internal,static,public,override,final,dynamic,abstract']=function(event)
{
    ...
    if( scope.keyword() !=='package' && scope.keyword() !=='class' )
    {
        if( this.strict() )this.error();
        if( this.current.value==='abstract' || this.current.value==='dynamic' )this.error();
        if( this.scope().keyword() !=='package' )this.add( new Scope('package', '(block)' ) );
        this.add( new Class() );
        scope = this.scope();
    }
    var arr = [event.__proxyTarget__.value];
    var self= this;
    var n;
    var description={};
    description[event.__proxyTarget__.value]=event.__proxyTarget__;
    this.loop(function(){
    ...
    if(this.strict() && o && this.scope().keyword() === 'class' && !this.scope().extends() )
...
syntax['class']=function( event )
{
    ...
    if( this.scope().parent().keyword() !=='package' )
    {
        this.error();
    }
...
syntax['function']= function(event){
    ...
    if( !this.strict() )
    {
        if( this.scope().keyword()==='rootblock' )
        {
            this.add( new Scope('package','(block)') );
        }
        if( this.scope().keyword()==='package' )
...
syntax["super"]=function (e)
{
    ...
    if( !fun.parent().extends() && this.strict() )this.error('No parent class inheritance');
...
Ruler.prototype.__strict__=true;
 ...
 * @returns {boolean|*}
 */
Ruler.prototype.strict=function( strict )
{
    if( typeof strict !== "undefined" )
    {
        this.__strict__ = !!strict;
    }
