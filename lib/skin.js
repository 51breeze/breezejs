...
function parse( elem, config , root, parent , filepath , filename , isTemplate )
{
    ...
    if( namespace )
    {
        ...
        //模板
        if( classname.substr(0,6) === 'Render')
        {
            if( !isTemplate )element.isTemplate = true;
            isTemplate = true;
        }
        ...
        else if( !isTemplate )
        {
            ...
            ////一个皮肤对象
            if( classname.indexOf( PATH.relative( config.project_path , config.project_skin_path ).replace('/','.') ) >= 0 )
            {
