...
function parse( elem, config , root, parent , filepath , filename )
{
    ...
    if( namespace )
    {
        ...
        }else
        {
            ...
            element.extends = '';
            element.attr.viewport=parent.isComponent ===true ? '('+parent.id+')' : '#'+(parent.attr.id || ( parent.attr.id = 'skin_'+Math.round( Math.random() * 100000 ) ) );
            element.package = namespace;
            element.filepath = filepath;
            element.script = [];
            element.isComponent = true;
            root.components.unshift( element );
        }
    }
    ...
    if( nodes.length > 0 )
    {
        for (var i in nodes)
        {
            var child= parse( nodes[i], config , element.name === 'component' ? element : root, element, filepath , filename );
            if( child )
            {
                if( child.id )
                {
                    ...
                    {
                        //实例化一个皮肤组件
                        var module = createModulObject('' ,'breeze.components.Skin', '' );
...
function start(view , config)
{
    ...
    skinObj.name= skinObj.attr.nodeName || 'div';
    var skinModule = createModulObject( skinObj.extends, skinObj.fullclassname, filepath );
    skinModule.script = makeModuleClass(config, skinModule ,skinObj.attr, skinObj.components || [],utils.trim( skinObj.script.join("") ) , makeSkinObject( skinObj ), 'skinObject' );
...
function makeModuleClass(config, classModule, attr, components, scriptContent , superParam ,paramName )
{
    var imports = [];
    var body=[];
    ...
    for (var classname in components )
    {
        ...
        makeSetProperty(body, component.attr, component.id , config );
    }
...
function makeSetProperty(body, attr, ref, config )
{
    for( var p in attr )
    {
        ...
        }else if( v.charAt(0) !=='@' && !/^\d+$/.test(v) && /^[\w\#]/.test(v)  )
        {
            v = '"'+v+'"';
        }
        body.push( ref+'.'+p+'='+v+';\n');
