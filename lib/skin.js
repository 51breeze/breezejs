...
//private
function __toString(skin, notContainer , parent )
{
    ...
    //如果组件在子级中
    if( skin.isComponent === true && parent )
    {
    ...
    for (var c in children)
    {
        ...
        {
            content += __toString(child, false, skin);
        } else
        {
 ...
 * @returns {*}
 */
function getClassPropertyDesc(name, currentModule, context )
{
    if( currentModule.description )
    {
        ...
        do{
            ...
            if( parent.inherit )
            {
                var parentClass = parent.inherit;
...
function parseBinable(attrValue, attrName, skinObject, context )
{
    if(attrValue && attrValue.slice(0,2) === '{{' && attrValue.slice(-2) === '}}' )
    {
        ...
        binding.bind.push({id:skinObject,attr:attrName,name:refname,flag:(attrValue[1]||'').toLowerCase()!=="false"});
        return "";
    }
    return attrValue;
}
 ...
 */
function parseAttributes(elem, skinObject, module, context )
{
    //属性
    ...
    for(var p in attrs)
    {
        ...
        //如果不是id并且有描述则检查
        if( ( isNew || skinObject.isProperty===true ) && attrName !=='id' && module.description )
        {
            ...
            {
                var desc = getClassPropertyDesc(attrName, module, context);
                if( desc )
                {
        ...
        if( attrName.toLowerCase()==='id' )
        {
            ...
            var context = hash === true ? context : module;
            if ( context.hash.hasOwnProperty(attrValue) )
            {
                throw new Error('"' + attrValue + '" has already been declared');
            }
            context.hash[ attrValue ] = hash
        }
        if( hash !== true )
        {
...
function parse(elem, context, module , parent )
{
    ...
    if ( name == null || name==='style' )
    {
        ...
        if( script )
        {
            if( name==='style' )
            {
                context.style.push( script );
            }
            //CDATA
            else if(name == null) {
                context.script.push(script);
            }
        }
        return null;
    }
    ...
    if( name === 'text' )
    {
        var value = utils.trim( elem.text() );
        if( parent.isSkin === true && value )
        {
    ...
    else if( elem.namespace() )
    {
        ...
        //尝试使用组件
        {
            ...
            skinObject.isComponent = true;
            skinObject.isSkin=fullname.indexOf( context.package )===0;
            if( skinObject.isSkin )
            {
                start( fullname,  context.config, context.project,  context.syntax,  context.loadModuleDescription  );
            }else{
                skinObject.description = context.loadModuleDescription(context.syntax, fullname, context.config, context.project, context.filepath );
            }
            module = skinObject;
        }
    ...
    //解析属性
    parseAttributes(elem, skinObject, module, context);

    //如果是一个组件，就必须设置id
    if( skinObject.isComponent ===true && !skinObject.id )
    {
    ...
    if( nodes.length > 0 )
    {
        ...
        for (var i in nodes)
        {
            ...
            //如果是属性稍后会参数处理
            if( child && child.isProperty !== true )
            {
                if( child.id )
                {
                    if( !skinObject.id )
                    ...
                    skinObject.children.push( child.id );
...
function setPropertyMethod( name, value, skinObject, context , desc )
{
    ...
    if( desc )
    {
        ...
        {
            throw new ReferenceError(attrName+' is not setter');
        }

        var id = skinObject.module.id;
        if (isset)
        {
            context.initProperty.push(id + '.' + name + '=' + value);
        } else
        {
            if( value instanceof Array )
            {
                context.initProperty.push(id + '.' + name + '(' + value.join(',') + ')');
            }else{
                context.initProperty.push(id + '.' + name + '(' + value + ')');
            }
        }
...
function makeScript( classModule )
{
    ...
    for (var classname in components )
    {
        ...
        {
           // makeSetProperty(setProperty, component.attr, component.id, classModule.config);
        }
    }
