...
function makeSkin(view , config )
{
    ...
    for( var skinName in skinContents )
    {
        ...
        for (var classname in skinObj.components)
        {
            ...
            if( component.script.length > 0 )
            {
                var parentClass = classname;
                classname+=uid();
                scriptContents.push({"classname":classname,"content":component.script.join('\n'),'package': view,'extends':parentClass,'filepath':skinObj.path});
            ...
            for(var p in component.attr )
            {
                var n=p;
                if( p==='skin' )n='setSkin';
                initialize.push('Reflect.apply( Reflect.get('+ref+', "'+n+'") ,'+ref+', '+component.attr[p]+')' );
            }
        }
