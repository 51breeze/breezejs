...
function getSkinPath(project)
{
    console.log( project )
   if( !project.child )return null;
   if( !project.child.hasOwnProperty('skin') )
   {
       return getSkinPath(project.child);
   }
   return project.child.skin.path;
}
...
function parse( elem, config , root, parent , filepath , filename , isTemplate )
{
    ...
    if( namespace )
    {
        ...
        }else if( config.project_skin_path.indexOf( namespace.replace('.','/') ) >= 0 )
        {
