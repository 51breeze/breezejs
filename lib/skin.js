...
function parse( elem, config , root, parent , filepath , filename , isTemplate )
{
    ...
    for(var p in attrs)
    {
        ...
        if( attrValue.slice(0,2) === '{{' && attrValue.slice(-2) === '}}' )
        {
            ...
            var binding = root.bindable[ attrValue[0] ] || (root.bindable[ attrValue[0] ]={});
            (binding.name || (binding.name=[]) ).push( {"attrName":attrName,"id":element.id} );
    ...
    }else if( element.id && element !== root)
    {
        ...
        module.param = makeSkinObject( element );
...
function makeModuleClass(config, classModule, attr, components, scriptContent ,styleContent, skinObject ,paramName )
{
    ...
    if( skinObject.bindable )
    {
        ...
        for( var p in skinObject.bindable )
        {
            ...
            item.varname = '__bind'+p+'__';
            bindableContent.push('var '+item.varname+' = new Bindable(this,"'+p+'");\n' );
            for(var i in item.name){
               bindableContent.push(item.varname+'.bind('+item.name[i].id+',"'+item.name[i].attrName+'","'+p+'");\n' );
           }
        }
        body.push(
