...
const createSkin = require('./skin.js');
    ...
    while ( it.next )
    {
        ...
        } else if( it.next.id === '(identifier)' || (it.next.id === '(keyword)' && it.current.value==='.') )
        {
            ...
            if ( desc && desc.type !=='*' && !(desc.referenceType ==='JSON' || desc.referenceType ==='Object') )
            {
                ...
                {
                    desc = getClassPropertyDesc(it, getmodule( getImportClassByType(classmodule, desc.type) ) , isstatic ? 'static' : 'proto', classmodule);
                }
                type = desc.type;
                property.descriptor = desc;
                property.type = type;
...
function getImportClassByType(classmodule, type )
{
    ...
    return globals.hasOwnProperty(type) ? type : null;
}
 ...
 * @returns {string}
 */
function makeClass(classname,inherit,attr)
{
    ...
    var init_attr = [];
    var p;
    if( attr.initialize )for (p in attr.initialize)
    {
        init_attr.push('this.'+p+'='+attr.initialize[p]);
    }
    var _class = "(function(Object,"+inherit+"){function "+name+"(){"+inherit+".call(this);"+init_attr.join(";\n")+"}\n";
    ...
    if( attr.property )for (p in attr.property)
    {
       _class+=name+'.prototype.'+p+'='+attr.property[p]+';\n';
    }
    _class+='return '+name+';\n}(System.Object,System.'+inherit+'))';
        ...
        if( item instanceof Ruler.STACK && item.keyword() !=='metatype' )
        {
            ...
            if( classModule.inherit && item.qualifier() !== 'private' )
            {
                info=doPrototype( getmodule( getImportClassByType(classModule,  classModule.inherit ) ),function (module) {
                    ...
                    if( desc && desc[ item.name() ] && desc[ item.name() ].qualifier !== 'private' )
                    {
            ...
            var metatype='';
            if(desc && desc.metatype )
            {
                metatype = metaTypeToString( desc.metatype );
                metatype = Utils.executeMetaType(metatype);
                switch ( metatype.type )
                {
                    case 'Skin' :
                        metatype =  metatype.param.source;
                        var _skin = createSkin( metatype , config );
                        skinContents.push(makeClass(_skin.filename,'Skin',{
                            "initialize":{
                                "__skins__":JSON.stringify(_skin.skin)
                            }
                        }));
                        requirements.Skin=true;
                        styles = styles.concat( _skin.style );
                    break;
                }
            }
...
function getNamespaceClassName( classname ) {
 ...
 * 开始生成代码片段
 */
function start(config, makeModules, descriptions)
{
    ...
    for(var p in descriptions)
    {
        ...
        for (var i in o.import)
        {
           if( globals.hasOwnProperty( o.import[i] )  )
           {
               if( i !== o.import[i] )
               {
                   str += 'var ' + getNamespaceClassName(i) + '=' + o.import[i] + ';\n';
               }
           }else
           {
               str += 'var ' + getNamespaceClassName(i) + '=$define("' + getmodule( getImportClassByType(o, i) ).fullclassname + '");\n';
           }
        }
        var descriptor = [];
        if( o.id!=='interface' )
    ...
    var content={};
    content.js = builder(config, code.join(''), requirements , skinContents.join("") );
    content.css = styles.join("");
    if( config.minify ==='on' )
    {
        content.js = uglify.minify(content.js, {mangle: true, fromString: true}).code;
        content.css = uglify.minify(content.css, {mangle: true, fromString: true}).code;
    }
    return content;
}
