...
function checkCallParameter(it, desc, property )
{
    ...
    if( param && parameters.length < param.length && !(param[0] ==='...' || param[0]==='*') )
    {
    ...
    while ( it.next )
    {
        if ( it.next instanceof Ruler.STACK )
        {
            ...
            }else if( it.current.type() === '(expression)' )
            {
                ...
                {
                    var props = [ property.thisArg ].concat( property.name );
 ...
 * @returns {string}
 */
function getSuperMethod( classmodule, superClass )
{
    var type = getImportClassByType(classmodule, superClass);
    return globals.hasOwnProperty(type) ? superClass+'.apply' : superClass+'.constructor.apply';
}
...
function operateMethodOfSuper(classmodule, thisvrg, desc, info)
{
    var method = classmodule.classname + '.super';
    var param = [info,thisvrg,'this','undefined'];
    var index = 3 ;
...
function expression( stack, classmodule ,  acceptType )
{
    ...
    if( acceptType && acceptType !=='*' && acceptType !=='Object' )
    {
        ...
        {
            console.log( valueDescribe , valueType )
 ...
 * @returns {string}
 */
function makeClass(classname,inherit,body,proto)
{
    var index = classname.lastIndexOf('.');
    var name = classname.substr(index+1);
    inherit = inherit || 'Object';
    var _class = "(function(Object,"+inherit+"){function "+name+"(){"+body+"}\n";
 ...
 * 开始生成代码片段
 */
function start(config, makeModules, descriptions , project, skinContents, styleContents )
{
    ...
    content.css = (styleContents||[]).join("");
    if( config.minify ==='on' )
    {
