...
function getmodule(name, module)
{
    ...
    var ref = obj[ classname.toLowerCase() ] || globals[classname];
    if(ref == null)return null;
    return ref.id == null && globals[classname] ? globals[classname] : ref;
}
...
function makeModule( stack )
{
    ...
    for ( var j in classModule.import )if( globals.hasOwnProperty(j) )
    {
        requirements[j]=true;
    }
 ...
 * 开始生成代码片段
 */
function start(config,needMakeModules,syntaxDescribe,modules)
{
    defineModules = modules;
    enableBlockScope = config.enableBlockScope==='on';
    ...
    var content = builder(config, code.join(''), requirements );
    if( config.minify ==='on' )
    {
