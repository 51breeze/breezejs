...
Iteration.prototype.nextIn=function(stack)
{
    ...
    }else
    {
        ...
        this.content.push({
            name:[],
            descriptor:null,
            thisArg:'var '+itn+' = Iterator('+parse( this.module, property)+');'+itn+'.seek() && ('+name+'='+itn+'.key)!=null;',
            expression:false,
            before:[],
            after:'',
            "super":null,
...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    var type;
    var desc;
    var property = {name:[],descriptor:null,thisArg:'',expression:false,before:[],after:'',"super":null,runningCheck:false,lastStack:null,isglobal:false,type:'*', funScope:it.stack.getScopeOf() };
    ...
    }else
    {
        ...
        if (isReference(it.current))
        {
            ...
            if ( !desc && !it.prev )
            {
                desc = getClassPropertyDesc(it,  getmodule(classmodule.type) ,'proto', classmodule );
        ...
        } else if( it.next.id === '(identifier)' || (it.next.id === '(keyword)' && it.current.value==='.') )
        {
            ...
            if ( desc && !(desc.type ==='*' || desc.type ==='JSON' || desc.type ==='Object') )
            {
                ...
                if( desc.notCheckType !== true )
                {
                    ...
                    desc = getClassPropertyDesc(it, refClassModule , isstatic ? 'static' : 'proto', classmodule, false );
                }
    ...
    if( it.next && it.next.value==='as' )
    {
        ...
        var before = property.before;
        property.before=[];
        property.thisArg = operateMethodOfCheckType(classmodule, typename.indexOf('.')>=0 ? "Internal.define('"+typename+"')" : typename, parse(classmodule,property), info );
        property.name=[];
        property.descriptor=null;
        property.runningCheck=false;
        property.isglobal=false;
...
function operateMethodOfNew( classmodule, express, desc, info )
{
    ...
    var index = 2 ;
    if( desc.descriptor && desc.descriptor.id==='class' )
    {
        if( checkInstanceType(classmodule, desc.descriptor, 'Error') )
        {
            var p = desc.param || (desc.param = []);
            if (p.length === 0)p.push("");
            if (p.length === 1)p.push(info);
            if (p.length === 2)p.push('"'+classmodule.filename+'"');
            if( desc.funScope )
            {
    ...
    if( desc.param )param.splice(index++,1,'[' + desc.param.join(",") + ']' );
...
function getClassPropertyDesc(it, refObj, name , classmodule , flag)
{
    if( refObj )
    {
        ...
        if( refObj[name] && Object.prototype.hasOwnProperty.call(refObj[name], prop) )
        {
            ...
            {
                //非全局模块和外部类需要检查权限
                if( !checkPrivilege(desc, refObj, classmodule  ) )
                {
...
function checkPrivilege(desc, inobject, currobject )
{
    if( currobject.type === inobject.type )
    {
        ...
        {
           var usenamespace = desc.__stack__.getScopeOf().define("use");
           if( !usenamespace || !usenamespace.hasOwnProperty(desc.privilege) )
           {
               return false;
           }
        }
 ...
 * @returns {string}
 */
function toValue( describe, config )
{
    var code=[];
    var properties=[];
    for( var p in describe )
    {
        ...
        //if(describe[p].privilege !=='public')

        if( "public,protected,internal,private".indexOf( describe[p].privilege ) < 0 )
        {
            item.push( '"ns":'+describe[p].privilege);

        }else
        {
            item.push( '"ns":'+config.context.namespace+'["'+describe[p].privilege+'"]' );
        }
...
{
    var descriptor = [];
    var ns =  config.context.namespace+'["'+ o.privilege+'"]';
    if( "public,protected,internal,private".indexOf( o.privilege ) < 0 )
    {
        ns= o.privilege;
    }
    ...
    descriptor = o.classname+'.T={' + descriptor.join(',') + '}'+";\n";
    descriptor += o.classname+'.S='+toValue(o.static, config )+";\n";
    descriptor += o.classname+'.P='+toValue(o.proto, config )+";\n";
    if( o.id !=='interface' )
    {
        descriptor += o.classname+'.construct='+o.constructor.value+";\n";
        descriptor +=o.classname+'.construct.prototype=Object.create( Object.prototype );\n';
        descriptor +=o.classname+'.construct.prototype.constructor='+o.classname+';\n';
    }
    return descriptor;
}
...
function makePrivateModuleClass( description, config )
{
    ...
    for(var p in declared )
    {
        ...
        {
            var o = declared[p];
            str="var "+o.classname+" = (function(){";
            str += 'return '+buildModuleStructure( o , config )+';\n';
            str += '}());\n';
            return str;
        }
    }
...
{
    var context = config.context.package;
    var str = '(function('+o.classname+'){\n';
    for (var i in o.import)
    {
        ...
        {
            var refClassname = getmodule( getImportClassByType(o, i) ).fullclassname;
            var selfPkg = o.package;
            if( refClassname.substr(0, selfPkg.length ) === selfPkg )
            {
                str += 'var ' + i + '=' + context +"." + refClassname.slice(selfPkg.length+1)+';\n';
    ...
    if( o.id==="namespace" )
    {
        ...
        {
            str+=context+"."+n+"=new Namespace('"+o.namespaces[n].value+"');\n";
        }
        return str;
    }
    ...
    var refClass=context+"."+o.classname;
    refClass+=" || ("+refClass+"=new "+(o.id==='interface' ? "Interface" : "Class")+"())";
    str += '}( '+refClass+' ));\n';
    return str;
}
...
function start(config, makeModules, descriptions , project )
{
    ...
    code= bulidPackages( packages , config.context.package );
    var builder = require( '../javascript/builder.js');
    return builder(config, code, requirements  );
}

function bulidPackages( packages, context , name )
{
    ...
    for(var n in packages )
    {
        if( n !== "package" )
        {
            child+=bulidPackages( packages[n] , context, n);
        }
    }
