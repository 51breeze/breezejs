...
function makeModule( stack , classModule, config )
{
        ...
        for(;j<len;j++)
        {
            if( content[j] instanceof Ruler.STACK )
            {
                ...
                }else if( content[j].keyword()==="namespace" )
                {
                    ...
                    console.log( classModule.namespaces[ content[j].name() ].value );
    ...
    //声明在包外的类
    if( stack.parent() && stack.parent().keyword()==="rootblock" )
    {
    ...
    for ( ; i< len ; i++ )
    {
        ...
        if( item instanceof Ruler.STACK && item.keyword() !=='metatype' )
        {
            ...
            {
                classModule.namespaces[ content[j].name() ].value=getNamespaceValue( item , classModule );
                continue;
            }
