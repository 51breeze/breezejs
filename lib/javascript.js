...
function getDescriptorOfExpression(it, classmodule)
{
    ...
    var property = {name:[],descriptor:null,thisArg:'',expression:false,before:[],after:'',"super":null,runningCheck:false,
        lastStack:null,isglobal:false,type:'*', funScope:it.stack.getScopeOf() };
...
function operateMethodOfGet(classmodule, thisvrg, desc, props ,info, before)
{
    ...
    /*if( desc.descriptor && desc.descriptor.owner && desc.descriptor.owner === classmodule.fullclassname )
    {
        console.log( desc.descriptor.owner )
    }*/
    ...
    if( !referType && desc && desc.funScope )
    {
     ...
     if( referType && !(referType ==="*" || referType ==="void" || referType ==="Object") )
     {
         ...
         if (  referModule.id==="class" )
         {
             ...
             {
                 var callparam = desc.param ? desc.param.slice(0) : [];
                 callparam.unshift( thisvrg );
                 if( props.length > 0 )
                 {
                     return referType+".prototype."+( props[0].replace(/[\"\']+/g,"") )+".call("+callparam.join(",")+")";

                 }else
                 {
                     return thisvrg+".call("+callparam.join(",")+")";
                 }
             }
         }
     }
...
function makeModule( stack , classModule, config )
{
    ...
    if( classModule.id !=='interface' && !isstatic )
    {
        ...
        props = '\nthis['+classModule.classname+'.token]={'+event.props.join(',')+'};\n';
 ...
 * @returns {string}
 */
function toValue( describe, config , prefix )
{
    var code=[];
    var properties={};
...
function buildModuleStructure( o , config )
{
    ...
    descriptor.push(o.classname+'.ns='+ ns);
    descriptor.push(o.classname+'.token=Symbol("'+o.classname+'")');
    descriptor.push(o.classname+'["extends"]='+ o.inherit);
    descriptor.push(o.classname+'["package"]="' + (o.package || "") + '"');
    descriptor.push(o.classname+'.classname="' + o.classname + '"');
    descriptor.push(o.classname+'.abstract='+(!!o.isAbstract) );
    descriptor.push(o.classname+'.filename="' + o.filename + '"');
    descriptor.push(o.classname+'.implements=[' + o.implements.join(',') + ']');
    descriptor.push(o.classname+'.final=' + !!o.isFinal);
    descriptor.push(o.classname+'.dynamic=' + !!o.isDynamic);
    descriptor.push(o.classname+'.id="' + o.id+'"'  );
    descriptor =  descriptor.join(';\n')+";\n";
    descriptor += toValue(o.static, config , o.classname+'.static' );
    descriptor += toValue(o.proto, config , o.classname+'.proto' );
    descriptor += o.classname+'.constructor='+(o.id ==='class' ? o.constructor.value : null)+";\n";
    return descriptor;
}
