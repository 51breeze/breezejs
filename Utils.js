...
(function( undefined )
{
    ...
    var notattr = /<(\w+)(?!=\{attr\s+([\w\s\+\.]+)\s*\})>/;
    var blank=/\s+/g;

    /**
     * 根据 options 中的元素生成皮肤
     * {attr head.style} 替换为  options.attr.head.style 的内容
     * {attr lable+title} 替换为  merge(options.attr.lable,options.attr.title) 的内容
     * {template contents} 替换为 options.template.contents 的内容
     *
     * @param string group
     * @param object options
     * @returns {string}
     */
    Utils.createSkin=function( group , options )
    {
        ...
        if( !options.attached )
        {
            for (var name in options.template)options.template[name] = options.template[name].replace(notattr, "<$1 {attr " + name + "}$2>");
            options.attached=true;
        }
        if( !options.template[group] )
            throw new Error('invalid group name in Utils.createSkin');
        return options.template[ group ].replace(refvalue,parser);
    }
